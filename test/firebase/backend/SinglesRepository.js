...
export default class SinglesRepository {
  ...
  update(id, newData) {
    const singles = require(this.dataFile);
    const singleToBeUpdated = singles.find(
      (single) => single.id === parseInt(id)
    );
  ...
  delete(id) {
    ...
    const singleToBeDeleted = singles.find(
      (single) => single.id === parseInt(id)
    );
  ...
  commitChanges(singles) {
    fs.writeFileSync(
      path.join(__dirname, this.dataFile),
      JSON.stringify(singles, null, '\t')
    );
  }
  ...
  getUserByEmail(email) {
    const singles = require(this.dataFile);
    return singles.find((single) => email === single.email);
  }
}
