...
export default function EmailReg() {
  ...
  async function handleSubmit(e) {
    ...
    if (isValid) {
      ...
      .then((result) => {
        alert("Sikeres email regisztráció. Email cím: " + result.user.email);
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  return (
    <main>
      <h1 className="mt-4 mb-4">Regisztráció email címmel</h1>
      <form
      ...
      <div className="mt-2"></div>
      {user && <div className='mt-3'>Bejelentkezve: {user.displayName}</div>}
    </main>
  );
}
